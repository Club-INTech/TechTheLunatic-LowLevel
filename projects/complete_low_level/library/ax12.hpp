...
#define AX_BROADCAST            0xFE        // Utilise l'ID 0xFE pour envoyer une commande � tous les AX12
...
private:
    ...
    // M�thode permettant d'envoyer un paquet lisible par l'AX12
    void sendPacket(uint8_t datalength, uint8_t instruction, uint8_t *data)
    {
    	/* datalength : nombre de param�tres utilis�s avec la commande (taille du tableau data)
    	 * instruction : commande donn�e � l'AX12 (cf liste de DEFINE ci-dessus)
    	 * data : tableau contenant les param�tres allant avec la commande
    	 */
     ...
     * L'octet renvoy� est le code d'erreur donn� par l'AX12 plus le code d'erreur de la fonction :
     * les bits 0 � 6 sont r�serv�s � l'erreur de l'AX12, le bit 7 est pass� � 1 si une erreur suppl�mentaire intervient
     * les eurreurs suppl�mentaires possibles sont : id incorrect, checksum incorrect, taille du message incorrecte, pas de message du tout
    */
...
**/
    ...
    void readData(uint8_t regstart, uint8_t reglength, unsigned char *answer)
    {
        ...
        while (buffer != 255)
        {
            Serial_AX12::read_char(buffer, 100); //attente du s�parateur de trame 0xFF
        }
        while (buffer == 255)
        {
            Serial_AX12::read_char(buffer, 100); //�vacuation du s�parateur et de l'id
        }
        Serial_AX12::read_char(buffer, 100); //taille des donn�es restantes � lire (nbDonn�esDemand�es + 2 : avec toss_error et checksum)
        uint8_t length = buffer - 2; //taille des donn�es utiles
        Serial_AX12::read_char(buffer, 100);  //�vacuation du toss_error
        for (uint8_t f = 0; f < length; f++)
        {
            Serial_AX12::read_char(answer[f], 100); //lecture des donn�es
        }
        Serial_AX12::read_char(buffer, 100);    //�vacuation du checksum
        Serial_AX12::disable_rx();        //d�sactiver la s�rie entrante
        Serial_AX12::enable_tx();          //r�activer la s�rie sortante
    }
...
public:
    ...
    /*
     * Tente de r�animer un AX12 mort.
     * L'argument baud_rate � passer en argument est le baud rate normal
     * de fonctionnement de la s�rie (9600)
     */
    void reanimationMode(uint16_t baud_rate)
    {
        ...
        // Une fois que le signal de reset a �t� re�u, l'AX12 �coute � 1.000.000 bps.
        // Donc � ce baud rate, on reflash le baud rate d'�coute de l'AX12.
...
//				writeDataB(AX_BAUD_RATE, 1, uint8_t(2000000/baud_rate - 1));
        ...
        // Si l'id est diff�rente du broadcast, alors on la reflash.
    ...
    /// R�initialisation de l'ID de l'AX12
    ...
    /// D�sasservissement d'un AX12 branch�.
    void unasserv()
    {
    ...
    // Changement de la limite de temp�rature
    ...
    // Lecture de la position courante, en degr�s
    uint16_t getPositionDegres()
    {
        return ((uint32_t) 300 * getPosition_0_1023()) / 1023;
    }

    // Retourne si l'AX-12 est en d�placement vers sa consigne
    bool isMoving()
    {
        uint8_t mov[1];
